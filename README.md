...
The Magic SOC sensor (`vehicle.magic_soc`) predicts battery state during both driving and charging. During driving, it uses odometer distance and a learned energy consumption rate to estimate drain when BMW stops sending SOC updates mid-drive. During charging, it delegates to the Predicted SOC engine (energy-based) so you get a single sensor that always shows the best available estimate.

> **Note:** Magic SOC is disabled by default. To enable it, go to **Configure > Settings** and toggle on **Enable Magic SOC**. The integration reloads automatically when settings change.

### How It Works

- **Driving**: `predicted = last_soc - ((distance_km * consumption_rate + aux_energy) / capacity * 100)`
- **Auxiliary power compensation**: When BMW sends `avgAuxPower` (HVAC, heating, pumps), this energy is integrated over the trip duration and added to the distance-based prediction. This means winter heating or summer A/C is accounted for in real-time, and the learned consumption rate stays clean (pure driving efficiency). If `avgAuxPower` is never received, the system behaves as pure distance-based prediction. Aux power extrapolation is capped at 10 minutes of staleness to prevent unbounded phantom energy, and readings above 10 kW or below 0 are rejected as bogus.
- **Charging**: Delegates to the Predicted SOC engine (energy-based, same as the `vehicle.predicted_soc` sensor)
- **Not driving/charging**: Shows BMW-reported SOC (passthrough)
- **BMW sends fresh SOC during driving**: Re-anchors prediction to new value (aux accumulator resets)
- **GPS distance fallback**: When the odometer (`travelledDistance`) isn't advancing during driving, GPS coordinates are used to estimate distance traveled via haversine. GPS jumps > 2 km (tunnel/signal loss) are rejected. Odometer is always preferred when available.

### Learning

- Default: 0.21 kWh/km (BMW BEV fleet average)
- **Model-aware defaults**: Known BMW EV models get a model-specific default consumption rate:

  | Model | kWh/km | Model | kWh/km |
  |-------|--------|-------|--------|
  | iX1 | 0.17 | i4 | 0.19 |
  | iX2 | 0.17 | i4 M50 | 0.21 |
  | iX3 | 0.20 | i5 | 0.20 |
  | iX | 0.23 | i5 M60 | 0.22 |
  | iX xDrive40 | 0.22 | i7 | 0.24 |
  | iX xDrive50 | 0.24 | i7 xDrive60 | 0.25 |
  | iX M60 | 0.26 | i7 M70 | 0.26 |

  Unknown models fall back to the 0.21 kWh/km fleet average.

- **Adaptive learning rate**: The first trip uses 100% of the measured value (fully replacing the default). Subsequent trips converge to the normal 20% EMA rate after 5 trips: `rate = max(0.2, 1/(trip_count+1))`. This means the sensor immediately uses your vehicle's actual consumption instead of slowly converging over ~10 trips.
- **Distance-weighted learning**: Short trips contribute less to learning than long trips. A 5 km trip gets ~17% of the normal learning weight, while trips of 30 km or more get full weight. This prevents noisy city trips from corrupting the learned rate.
- Minimum trip distance: 5 km, minimum SOC drop: 2%
- Consumption range: 0.10-0.40 kWh/km (outliers rejected)

### Driving Session Triggers

A driving session (prediction mode) is anchored when any of these conditions is detected:
- **BMW `isMoving`** transitions to `True` (including after HA restart when previous state is unknown)
- **GPS-derived motion** detected (including after HA restart)
- **Odometer increase** without an active session — if `travelledDistance` goes up, the car is definitively driving

This ensures the first drive after an HA restart is captured even when the previous motion state is unknown.

### Mileage-based isMoving Fallback

When no GPS data is available for a vehicle, the `isMoving` binary sensor uses odometer changes as evidence of movement. If the odometer increased within the last 7 minutes, the vehicle is considered moving. GPS remains the primary source when available.

### Resilience to Long SOC Gaps

BMW may stop sending `batteryManagement.header` (SOC) updates for extended periods. When this happens, the coordinator's descriptor eviction would normally remove the SOC and battery capacity from memory, preventing new driving sessions from anchoring. To handle this:

- The last known BMW SOC and battery capacity are cached and **persisted across restarts**
- When a driving trip starts and the live descriptor has been evicted, the cached values are used as fallback to anchor the driving session
- The `soc_data_age` attribute shows how stale the underlying SOC data is

### State Attributes

The Magic SOC sensor exposes extra attributes for visibility:
- `prediction_mode`: "driving", "charging", "continuity", "passthrough", or "passthrough (PHEV)"
- `learned_consumption_kwh_km`: Learned consumption rate (only shown after first trip)
- `trip_count`: Number of trips used for learning
- `learning_rate`: Current adaptive learning rate
- `default_consumption_kwh_km`: Model-based or fleet-average default
- `driving_distance_km`: GPS-accumulated distance during active driving (only shown when GPS fallback is in use)
- `aux_energy_kwh`: Accumulated auxiliary energy during driving (HVAC, heating, etc.) — only shown when `avgAuxPower` data is available
- `avg_speed_kmh`: Average trip speed in km/h (only shown during active driving when odometer data is available)
- `soc_data_age`: Time since last BMW SOC update (shown as minutes or hours)

### Stale Session Expiry

After a Home Assistant restart, persisted driving sessions older than 4 hours are discarded. This prevents stale predictions from sessions that were active hours before the restart.

### BEV Only

The Magic SOC sensor is only created for pure electric vehicles (BEV). PHEVs do not get this sensor because the hybrid powertrain makes distance-based consumption prediction unreliable.

### Reset Button

A "Reset Magic Learning" button (BEV only) resets the learned rate to the model default (or fleet average). After reset, the adaptive learning rate starts over at 100% for the next trip.
