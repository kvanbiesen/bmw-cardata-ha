...
- **Dampened charging level sync**: During charging, `charging.level` applies a dampened correction (50%) when it is lower than the prediction, smoothing out drift without causing a staircase effect. Stale header values are still blocked from re-anchoring, preserving session integrity
